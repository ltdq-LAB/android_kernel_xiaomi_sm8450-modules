...
static uint8_t xdata_tmp[NVT_XDATA_NUM_MAX * sizeof(int16_t)] = {0};
static int32_t xdata[NVT_XDATA_NUM_MAX] = {0};
static int32_t xdata_pen_tip_x[NVT_PEN_DATA_NUM_MAX] = {0};
static int32_t xdata_pen_tip_y[NVT_PEN_DATA_NUM_MAX] = {0};
static int32_t xdata_pen_ring_x[NVT_PEN_DATA_NUM_MAX] = {0};
static int32_t xdata_pen_ring_y[NVT_PEN_DATA_NUM_MAX] = {0};
...
void nvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	sample_count = (size_t)ts->x_num * ts->y_num;
	if (!sample_count || sample_count > ARRAY_SIZE(xdata)) {
		NVT_ERR("invalid metadata dimensions %u x %u\n",
			ts->x_num, ts->y_num);
		return;
	}
	data_len = sample_count * sizeof(int16_t);
	total_len = dummy_len + data_len;
	read_len = ALIGN(total_len, XDATA_SECTOR_SIZE);
	if (read_len > sizeof(xdata_tmp)) {
		NVT_ERR("metadata read length %zu exceeds buffer %zu\n",
			read_len, sizeof(xdata_tmp));
		return;
	}
...
{
	size_t sample_count = (size_t)ts->x_num * ts->y_num + TOUCH_KEY_NUM;

	if (!buf || !m_x_num || !m_y_num ||
	    sample_count > ARRAY_SIZE(xdata))
		return;

    *m_x_num = ts->x_num;
    *m_y_num = ts->y_num;
    memcpy(buf, xdata, sample_count * sizeof(*buf));
}
...
void nvt_read_get_num_mdata(uint32_t xdata_addr, int32_t *buffer, uint32_t num)
{
	...
	head_addr = xdata_addr - (xdata_addr % XDATA_SECTOR_SIZE);
	dummy_len = xdata_addr - head_addr;
	if (!buffer || !num || num > NVT_XDATA_NUM_MAX) {
		NVT_ERR("invalid metadata sample count %u\n", num);
		return;
	}
	data_len = (size_t)num * sizeof(int16_t);
	total_len = dummy_len + data_len;
	read_len = ALIGN(total_len, XDATA_SECTOR_SIZE);
	if (read_len > sizeof(xdata_tmp)) {
		NVT_ERR("metadata read length %zu exceeds buffer %zu\n",
...
out_restore:
	if (in_test_mode)
		nvt_change_mode(NORMAL_MODE);
	if (pen_mode_attempted) {
		cleanup_ret = nvt_set_pen_normal_mode();
		if (!ret && cleanup_ret)
			ret = -EAGAIN;
		cleanup_ret = nvt_check_fw_reset_state(RESET_STATE_NORMAL_RUN);
		if (!ret && cleanup_ret)
			ret = -EAGAIN;
	}
out_unlock:
...
static int nvt_xiaomi_lockdown_info_show(struct seq_file *m, void *v)
{
	...
	ret = nvt_read_lockdown_info(ts->lockdown_info);
	if (ret < 0) {
		NVT_ERR("can't get lockdown info: %d\n", ret);
		mutex_unlock(&ts->lock);
		return ret;
	} else {
		WRITE_ONCE(ts->lkdown_readed, true);

...
#define NVT_DIFF_TEST_FRAME_MAX 2550
...
static struct proc_dir_entry *NVT_proc_aftersales_test_entry = NULL;
...
static int nvt_restore_normal_firmware(void)
{
	int ret;
	...
	if (ret < 0)
		NVT_ERR("restore normal firmware failed: %d\n", ret);
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	//---Download MP FW---
	mp_fw_attempted = true;
	if (nvt_get_dbgfw_status()) {
		if (nvt_update_firmware(DEFAULT_DEBUG_MP_NAME) < 0) {
			NVT_ERR("use built-in fw");
			if (nvt_update_firmware(ts->mp_name) < 0) {
	...
	ret = nvt_restore_normal_firmware();
	mp_fw_attempted = false;
	if (ret < 0)
		goto out_unlock;
...
out_restore:
	if (in_test_mode)
		nvt_change_mode(NORMAL_MODE);
	if (mp_fw_attempted) {
		restore_ret = nvt_restore_normal_firmware();
		if (restore_ret < 0)
			ret = restore_ret;
	}
out_unlock:
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_ain(struct device_node *np, const char *name, uint8_t *array,
		int32_t size, uint32_t array_size, uint32_t channel_count)
{
	uint32_t tmp[NVT_XDATA_DIM_MAX];
	int32_t len, ret;
	int32_t i;

	if (size <= 0 || (uint32_t)size > ARRAY_SIZE(tmp) ||
	    (uint32_t)size > array_size ||
	    !channel_count) {
		NVT_ERR("invalid %s size=%d max=%u channels=%u\n",
			name, size, array_size, channel_count);
		return -1;
	}

	len = of_property_count_u32_elems(np, name);
	if (len < 0 || len != size) {
		NVT_ERR("error find %s. len=%d expected=%d\n", name, len, size);
		return -1;
	} else {
		NVT_LOG("%s. len=%d\n", name, len);
		ret = of_property_read_u32_array(np, name, tmp, size);
		if (ret) {
		...
		for (i = 0; i < size; i++) {
			if (tmp[i] != 0xFF && tmp[i] >= channel_count) {
				NVT_ERR("invalid %s[%d]=%u, channels=%u\n",
					name, i, tmp[i], channel_count);
				return -1;
			}
		}
		...
		nvt_print_result_log_in_one_line(array, size);
		printk("\n");
#endif
	}
		...
		int32_t x_ch, int32_t y_ch)
{
	...
	if (x_ch <= 0 || y_ch <= 0 || Key_Channel > X_Y_DIMENSION_MAX ||
	    (uint32_t)x_ch >
		(X_Y_DIMENSION_MAX - Key_Channel) / (uint32_t)y_ch) {
		NVT_ERR("invalid %s dimensions %dx%d keys=%u\n",
			name, x_ch, y_ch, Key_Channel);
		return -1;
	}

	size = (uint32_t)x_ch * (uint32_t)y_ch + Key_Channel;
	len = of_property_count_u32_elems(np, name);
	if (len < 0 || (uint32_t)len < size) {
		NVT_ERR("error find %s. len=%d required=%u\n", name, len, size);
		return -1;
	} else {
		NVT_LOG("%s. len=%d required=%u\n", name, len, size);
		ret = of_property_read_u32_array(np, name, (uint32_t *)array, size);
		if (ret) {
			NVT_ERR("error reading %s. ret=%d\n", name, ret);
			return -1;
		}
		...
		uint32_t x_num, uint32_t y_num)
{
	...
	if (!x_num || x_num > NVT_XDATA_DIM_MAX ||
	    !y_num || y_num > NVT_XDATA_DIM_MAX ||
	    x_num > PEN_X_Y_DIMENSION_MAX / y_num) {
		NVT_ERR("invalid %s dimensions %ux%u\n", name, x_num, y_num);
		return -1;
	}
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	int32_t ic_x_cfg_size;
	int32_t ic_y_cfg_size;
	int32_t x_channel;
	int32_t y_channel;
	int32_t diff_test_frame;
	int32_t ret = -1;
#if TOUCH_KEY_NUM > 0
	int32_t ic_key_cfg_size;
#endif
	...
	if (ic_x_cfg_size <= 0 ||
	    (uint32_t)ic_x_cfg_size > ARRAY_SIZE(AIN_X) ||
	    ic_y_cfg_size <= 0 ||
	    (uint32_t)ic_y_cfg_size > ARRAY_SIZE(AIN_Y) ||
	    x_channel <= 0 || x_channel > NVT_XDATA_DIM_MAX ||
	    y_channel <= 0 || y_channel > NVT_XDATA_DIM_MAX ||
	    (uint32_t)ic_x_cfg_size >
		X_Y_DIMENSION_MAX / (uint32_t)ic_y_cfg_size ||
	    (uint32_t)x_channel >
		X_Y_DIMENSION_MAX / (uint32_t)y_channel) {
		NVT_ERR("invalid MP dimensions cfg=%dx%d channels=%dx%d\n",
			ic_x_cfg_size, ic_y_cfg_size, x_channel, y_channel);
		goto out;
	}

#if TOUCH_KEY_NUM > 0
	if (ic_key_cfg_size <= 0 ||
	    (uint32_t)ic_key_cfg_size > ARRAY_SIZE(AIN_KEY) ||
	    Key_Channel > ARRAY_SIZE(AIN_KEY)) {
		NVT_ERR("invalid MP key dimensions cfg=%d channels=%u\n",
			ic_key_cfg_size, Key_Channel);
		goto out;
	}
#endif

	if (nvt_mp_parse_ain(np, "AIN_X", AIN_X, ic_x_cfg_size,
			ARRAY_SIZE(AIN_X), x_channel))
		goto out;

	if (nvt_mp_parse_ain(np, "AIN_Y", AIN_Y, ic_y_cfg_size,
			ARRAY_SIZE(AIN_Y), y_channel))
		goto out;

#if TOUCH_KEY_NUM > 0
	if (nvt_mp_parse_ain(np, "AIN_KEY", AIN_KEY, ic_key_cfg_size,
			ARRAY_SIZE(AIN_KEY), Key_Channel))
		goto out;
#endif
	...
	if (nvt_mp_parse_u32(np, "PS_Config_Diff_Test_Frame", &diff_test_frame))
		goto out;
	if (diff_test_frame <= 0 ||
	    diff_test_frame > NVT_DIFF_TEST_FRAME_MAX) {
		NVT_ERR("invalid PS_Config_Diff_Test_Frame=%d (expected 1..%d)\n",
			diff_test_frame, NVT_DIFF_TEST_FRAME_MAX);
		goto out;
	}

	IC_X_CFG_SIZE = ic_x_cfg_size;
	IC_Y_CFG_SIZE = ic_y_cfg_size;
#if TOUCH_KEY_NUM > 0
	IC_KEY_CFG_SIZE = ic_key_cfg_size;
#endif
	X_Channel = x_channel;
	Y_Channel = y_channel;
	PS_Config_Diff_Test_Frame = diff_test_frame;
...
static int nvt_short_test(void)
{
	...
	/*---Download MP FW---*/
	if (nvt_get_dbgfw_status()) {
		ret = nvt_update_firmware(DEFAULT_DEBUG_MP_NAME);
		if (ret < 0) {
			NVT_ERR("use built-in fw");
			ret = nvt_update_firmware(ts->mp_name);
		}
	} else {
		ret = nvt_update_firmware(ts->mp_name);
	}
	if (ret < 0) {
		NVT_ERR("update mp firmware failed: %d\n", ret);
		ret = -EAGAIN;
		goto out_restore;
	}
...
{
	struct nvt_ts_data *nvt = READ_ONCE(ts);
	char tmp[5] = {0};
	int cnt;
	...
	return simple_read_from_buffer(buf, count, pos, tmp, cnt);
}
...
static ssize_t tp_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (!strncmp("short", tmp, 5)) {
		...
		if (mutex_lock_interruptible(&nvt->lock)) {
			retval = -ERESTARTSYS;
		} else {
	...
	switch (retval) {
	case 0:
		nvt->result_type = NVT_RESULT_PASS;
		break;
	case -EINVAL:
		nvt->result_type = NVT_RESULT_INVALID;
		break;
	default:
		nvt->result_type = NVT_RESULT_FAIL;
		break;
	}
...
int32_t nvt_mp_proc_init(void)
{
	...
	if (ret) {
		NVT_ERR("Allocate mp memory failed: %d\n", ret);

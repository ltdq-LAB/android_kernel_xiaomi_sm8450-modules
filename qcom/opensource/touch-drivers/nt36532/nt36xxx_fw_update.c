...
static bool nvt_fw_range_valid(size_t fwsize, size_t offset, size_t len)
{
	return offset <= fwsize && len <= fwsize - offset;
}

/* The Novatek partition size field stores length minus one. */
static bool nvt_bin_data_valid(size_t fwsize, uint32_t addr, uint32_t size)
{
	if (!size)
		return addr <= fwsize;

	return addr < fwsize && size < fwsize - addr;
}
...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	...
	size_t header_limit;
	uint32_t end;
	uint32_t info_sec_num = 0;
	uint32_t ovly_sec_num = 0;
	uint8_t ovly_info = 0;
	bool find_bin_header = false;

	if (!fwdata || !nvt_fw_range_valid(fwsize, 0, 0x29)) {
		NVT_ERR("firmware is too small for a Novatek header\n");
		return -EINVAL;
	}

	/* Find the header size */
	end = byte_to_word(fwdata);
	if (end < 0x30 || end > fwsize) {
		NVT_ERR("invalid header end 0x%08X for firmware size 0x%zx\n",
			end, fwsize);
		return -EINVAL;
	}
	...
	if (cascade_2nd_header_info) {
		if (end & 1) {
			NVT_ERR("cascade header end is not even: 0x%08X\n", end);
			return -EINVAL;
		}
		header_limit = end / 2;
	} else {
	...
	if (header_limit < 0x30 || (header_limit - 0x30) % 0x10) {
		NVT_ERR("invalid header section boundary 0x%zx\n", header_limit);
		return -EINVAL;
	}
	info_sec_num = (header_limit - 0x30) / 0x10;
	if (cascade_2nd_header_info)
		info_sec_num++;	/* cascade second-header terminator */
	...
	if (partition < ilm_dlm_num ||
	    partition > (NVT_READ_LEN - 2) / sizeof(uint32_t)) {
		NVT_ERR("invalid firmware partition count %u\n", partition);
		return -EINVAL;
	}
	...
	for (list = 0; list < partition; list++) {
		...
		if (list < ilm_dlm_num) {
			pos = list * 12;
			if (!nvt_fw_range_valid(fwsize, pos, 12) ||
			    !nvt_fw_range_valid(fwsize, 0x18 + list * 4, 4)) {
				NVT_ERR("truncated ILM/DLM header %u\n", list);
				return -EINVAL;
			}
			...
			if (ts->hw_crc)
				...
				if (!bin_map[list].size)
					bin_map[list].crc = 0;
				else if (nvt_bin_data_valid(fwsize, bin_map[list].BIN_addr,
							bin_map[list].size))
					bin_map[list].crc = CheckSum(&fwdata[bin_map[list].BIN_addr], bin_map[list].size);
				else {
					NVT_ERR("access range (0x%08X to 0x%llX) is larger than bin size!\n",
							bin_map[list].BIN_addr,
							(unsigned long long)bin_map[list].BIN_addr + bin_map[list].size);
					return -EINVAL;
				}
			} //ts->hw_crc
			if (list == 0)
		...
		if ((list >= ilm_dlm_num) && (list < (ilm_dlm_num + info_sec_num))) {
			...
			if (!nvt_fw_range_valid(fwsize, pos, 0x10)) {
				NVT_ERR("truncated info header at 0x%zx\n", pos);
				return -EINVAL;
			}
		...
		if (list >= (ilm_dlm_num + info_sec_num)) {
			...
			if (pos < bin_map[1].BIN_addr ||
			    !nvt_fw_range_valid(fwsize, pos, 0x10)) {
				NVT_ERR("invalid overlay header at 0x%zx\n", pos);
				return -EINVAL;
			}
		...
		if (!nvt_bin_data_valid(fwsize, bin_map[list].BIN_addr,
					bin_map[list].size)) {
			NVT_ERR("access range (0x%08X to 0x%llX) is larger than bin size!\n",
					bin_map[list].BIN_addr,
					(unsigned long long)bin_map[list].BIN_addr + bin_map[list].size);
...
void nvt_fw_update_deinit(void)
{
...
static int32_t update_firmware_request(const char *filename)
{
	...
	while (1) {
		...
		/* BIN Header Parser */
		ret = nvt_bin_header_parser(fw_entry->data,
				fw_need_write_size);
		if (ret) {
		...
		uint32_t SRAM_addr, uint32_t size, uint32_t BIN_addr)
{
	...
	while (size) {
		len = min_t(uint32_t, size, NVT_TRANSFER_LEN);
...
//				list, name, SRAM_addr, size, BIN_addr);
		...
		/* Check data size */
		if (!nvt_bin_data_valid(fwsize, BIN_addr, size)) {
			NVT_ERR("access range (0x%08X to 0x%llX) is larger than bin size!\n",
					BIN_addr, (unsigned long long)BIN_addr + size);
			ret = -EINVAL;
			goto out;
		}
...
static int32_t nvt_download_firmware_hw_crc(void)
{
	...
	while (1) {
		...
		if (cascade_2nd_header_info) {
			...
			ret = nvt_write_firmware(fw_entry->data,
					fw_need_write_size);
			if (ret) {
...
static int32_t nvt_download_firmware(void)
{
	...
	while (1) {
		...
		/* Start to write firmware process */
		ret = nvt_write_firmware(fw_entry->data, fw_need_write_size);
		if (ret) {
			NVT_ERR("Write_Firmware failed. (%d)\n", ret);
			goto fail;
		}
...
int32_t nvt_update_firmware(const char *firmware_name)
{
	...
	if (!strcmp(firmware_name, BOOT_UPDATE_FIRMWARE_NAME))
		selected_name = ts->lcd_id_value ? BOOT_UPDATE_FIRMWARE_NAME_TM :
			BOOT_UPDATE_FIRMWARE_NAME_CSOT;
	else if (!strcmp(firmware_name, MP_UPDATE_FIRMWARE_NAME))
		selected_name = ts->lcd_id_value ? MP_UPDATE_FIRMWARE_NAME_TM :
			MP_UPDATE_FIRMWARE_NAME_CSOT;

	NVT_LOG("firmware alias %s selects %s\n", firmware_name, selected_name);
